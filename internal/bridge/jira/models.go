...
// adfDocument represents Jira's Atlassian Document Format structure
//
// ADF is used for rich text in Jira Cloud (descriptions, comments)
// This is a simplified representation that extracts plain text
//
// Full ADF schema: http://go.atlassian.com/adf-json-schema
...
// extractDescription parses ADF (Atlassian Document Format) and extracts plain text.
// Returns empty string if description is null or cannot be parsed.
func extractDescription(raw json.RawMessage) string {
	...
	if err := json.Unmarshal(raw, &adf); err != nil {
		// Failed to parse ADF - return empty string
		return ""
	}
...
// extractTextFromADF extracts plain text from an ADF document structure
func extractTextFromADF(adf adfDocument) string {
